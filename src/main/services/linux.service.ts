...
export class LinuxService {
    ...
    // ARM64 Proton uses files/bin-arm64/wine. Its .NET host (mscoree) can only
    // probe for an x86_64 mono PE, so x86 .NET tools like IPA.exe cannot run on it.
    public isArm64Proton(): boolean {
        try {
            return this.getWinePath().includes(path.join("files", "bin-arm64", "wine"));
        } catch {
            return false;
        }
    }
